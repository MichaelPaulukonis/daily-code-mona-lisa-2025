  ...
  const shiftImage = (img, offsets = [0, 20, 40]) => {
    ...
    for (let y = 0; y < img.height; y++) {
      for (let x = 0; x < img.width; x++) {
        ...
        const redIndex = (((x + offsets[0]) % img.width) + y * img.width) * 4
        const greenIndex = (((x + offsets[1]) % img.width) + y * img.width) * 4
        const blueIndex = (((x + offsets[2]) % img.width) + y * img.width) * 4
